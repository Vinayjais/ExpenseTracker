  ...
  const premimuContainer2 =document.getElementById('premium-container2')
   ...
   IncomDownload.appendChild(downloadText2);
    ...
    downloadBtn.addEventListener('click' , async function (){
       ...
       await axios.get('http://13.51.161.135:4000/download',{headers:{"Authorization" : token}})
       .then((result) => {
    ...
    IncomDownload.addEventListener('click' , async function (){
      ...
      await axios.get('http://13.51.161.135:4000/downloadIncomes',{headers:{"Authorization" : token}})
      .then((result) => {
         window.location.href = result.data.url;
      })
      .catch((err) =>{
    ...
    await axios.get('http://13.51.161.135:4000/premium-user-leaderboard',{headers : {"Authorization" : token}})
    .then((result) => {
...
async function findPremium(){
       ...
       await axios.get('http://13.51.161.135:4000/is-premium-user', {headers : {"Authorization" : token}})
       .then(( response) => {
...
async function addnewexpense (){
  ...
  const  select = document.getElementById('select').value;
   ...
   await axios.post('http://13.51.161.135:4000/register-expense',expense,{ headers: {"Authorization" : token }})
     .then( resonse => {
   ...
   async function fetchData(){
        ...
        //  fetch expenses data---
          await axios.get('http://13.51.161.135:4000/expenses', { headers : {"Authorization" : token}})
          .then(results => {
            console.log(results);
          ...
          await axios.get('http://13.51.161.135:4000/incomes', { headers : {"Authorization" : token}})
          .then(results => {
            console.log(results);
          ...
          await axios.get('http://13.51.161.135:4000/balance', { headers : {"Authorization" : token}})
          .then(results => {
            console.log(results);
       ...
       // appending li element to ul *********************
        ...
        deletebtn.addEventListener('click', decreas);
         ...
         async function decreas(){
          ...
          await axios.post(`http://13.51.161.135:4000/decreas-exspense`,{amount,select},{headers:{"Authorization" : token}})
            .then(()=> {
        ...
        async function removeLi(){
            ...
            await axios.delete(`http://13.51.161.135:4000/${router}/${id}`)
              .then( result => {
               console.log('deleted..');
                tr.removeChild(li);
              })
...
/// reazor pau handler  
 ...
 async function buyPremiume(e){
   ...
   const response = await axios.get('http://13.51.161.135:4000/buy-premium', { headers: {"Authorization" : token }})
   
    var options ={
         "key": response.data.key_id,
         "order_id": response.data.order.id,
         "handler": async function (resonse){
             await axios.post('http://13.51.161.135:4000/premium-success',{
